...
// a function to normalize (scale down) the data
let scaleDown = (step) => {
 return {
  open: step.open / 138,
  high: step.high / 138,
...
console.log(scaleDown(rawData[0])) // check that our normalization works

// normalize all the raw data
...
net.train(trainingData, {
 learningRate: 0.005, //to not shoot past the value we're looking for
 errorThresh: 0.02, // lot of data, takes a lot to train, turn down the error threshold to not take too long
 // log: (stats) => console.log(stats)
})

console.log(net.run(trainingData[0]))

// to predict 2 or 3 steps ahead, we use net.forecast()
console.log(net.forecast([ // we give it an array of steps and it will predict the next 3 steps (array)
 trainingData[0][0],
 trainingData[0][1]
], 3).map(scaleUp)) // 3 is the number of steps
...
def liquefaction():
  ...
  test = int(input("""\n
    Pour le calcul du CRR, entrez ... 
      0   pour essai CPT
      1   pour essai SPT
      2   pour essai Vs
  """))
  while test != 0 and test != 1 and test != 2:
    print("Vous avez seulement les options suivantes:")
    test = int(input("""\n
    Pour le calcul du CRR, entrez ... 
      0   pour essai CPT
      1   pour essai SPT
  ...
  if test == 0:
    ...
    qc = float(input("qc: "))
    fs = float(input("fs: "))
    test = Liquefaction.CPT(qc, fs, contrainte_effective_initiale, contrainte_totale_initiale)
    print("Resultat qc1ncs = " + str(test.qc1ncs()) + "\n")
  elif test == 1:
    print("\nOn utilise un essai SPT. Entrez le nombre de coups N et % des fines FC dans la profondeur " + str(profondeur) + "m choisie.")
    N = float(input("N: "))
    FC = float(input("FC: "))
    test = Liquefaction.SPT(N, contrainte_effective_initiale, FC)
    print("Resultat N160cs = " + str(test.N160cs()) + "\n")
  elif test == 2:
    print("\nOn utilise la méthode Downhole. Entrez la vitesse sismique Vs et % des fines FC dans la profondeur " + str(profondeur) + "m choisie.")
    vitesse_sismique = float(input("Vs: "))
    FC = float(input("FC: "))
    test = Liquefaction.Vs(vitesse_sismique, contrainte_effective_initiale, FC)
    print("Resultat Vs1 = " + str(test.Vs1()) + "\n")

  FS = Liquefaction.Facteur_de_Securite(acceleration_max, contrainte_effective_initiale, contrainte_totale_initiale, profondeur, test, magnitude, densite_relative)
  FS.result_FS()
